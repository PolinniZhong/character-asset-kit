...
description: 把一个新角色（3D 卡通/盲盒手办风为主，也适配其他风格 profile）按企业级门径生产线做成可复用 AIGC 角色资产库——G0 立项规格卡、G1 白底母版、G2 五视图多视图、G3 表情、G4 细节特写、G5 营销胸像、G6 动作姿态、G7 单体道具与手持关系、G8 确定性拼板与角色设定商卡、G9 封存，以及扩展门 G10 微表情矩阵、G11 机位俯仰视线、G12 场景包、G13 风格变体、G14 LoRA 训练素材备料。当用户要求"建新角色/角色资产/角色库/character sheet/角色设定图板/多视图三视图/表情板/动作库/角色道具/角色商卡/角色 LoRA 数据集/角色一致性资产/按门径生产角色"时使用。内置确定性脚本（Pillow＋macOS Vision 抠图）负责落标准格、拼板、台账对账、训练集导出；生图由运行时 Agent 调图像生成模型完成。不用于写文章配图（那是风格型插画 skill 的职责）、不做真人写实照片、不承诺库内训练 LoRA。
---

# character-asset-kit · 角色资产生产线
